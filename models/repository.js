        ...
        if (params) {
            ...
            for (let object of objectsList) {
                for (let searchKey of searchKeys) {
                    let valueMatch = this.valueMatch(object[searchKey.key], searchKey.value);
                    if (object[searchKey.key].toLowerCase() === searchKey.value.toLowerCase() || valueMatch) {
            ...
            //if filteredAndSortedObjects is empty, put all the objects inside of it.
            ...
            for (let key of sortKeys) {
                if(key.asc)
                {
                    filteredAndSortedObjects.sort((x, y) => this.innerCompare(x[key.key], y[key.key]))
                }
                else
                {
                    filteredAndSortedObjects.sort((x, y) => this.innerCompare(y[key.key], x[key.key]))
                }
            }
